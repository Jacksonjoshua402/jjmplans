    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 pt-10 pb-10">
      ...
      <div className="text-center mb-8">
        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-cyan-500/10 border border-cyan-500/30 mb-4">
          ...
          <span className="text-xs font-bold tracking-[0.2em] text-cyan-400 uppercase">Sermon Archive</span>
        </div>
        <h1
        ...
        <p className="text-cyan-200/60 text-sm italic max-w-xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-8">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2.5 rounded-xl bg-slate-800/50 border border-slate-700 text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-cyan-500/40 focus:border-cyan-500/40 transition-all text-sm"
          />
        </div>
      ...
      {messages.length === 0 ? (
        <div className="text-center py-20 bg-slate-900/30 rounded-2xl border border-dashed border-slate-700">
          <div className="inline-flex h-16 w-16 rounded-2xl bg-cyan-500/10 items-center justify-center mb-4">
            <BookOpen size={28} className="text-cyan-400/60" />
          </div>
          <h3 className="text-cyan-200/70 font-semibold mb-1">No messages yet</h3>
          ...
          {Object.entries(groupedByMonth).map(([month, monthMsgs]) => (
            <div key={month}>
              <div className="flex items-center gap-3 mb-4">
                <h2 className="text-sm font-bold tracking-[0.15em] text-cyan-400/80 uppercase">{month}</h2>
...
function MessageListRow({ message, onView }: { message: MessageNote; onView: (m: MessageNote) => void }) {
  ...
  return (
    <div
      ...
      className="flex items-center gap-4 p-4 bg-slate-800/40 border border-slate-700/50 rounded-xl hover:border-cyan-500/40 hover:bg-slate-800/60 transition-all cursor-pointer group"
    >
      {/* Thumbnail */}
      <div className="flex-shrink-0 w-20 h-14 rounded-lg overflow-hidden bg-slate-700">
        {message.coverImage ? (
          ...
          <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-cyan-500/20 to-sky-500/20">
            <BookOpen size={18} className="text-cyan-400/50" />
          </div>
        )}
      </div>
      ...
      <div className="flex-1 min-w-0">
        <h3 className="text-sm font-semibold text-cyan-100 group-hover:text-cyan-300 transition-colors truncate">
          {message.title}
        </h3>
        <div className="flex items-center gap-3 mt-1 text-xs text-slate-400">
          {message.speaker && <span>{message.speaker}</span>}
          <span>· {formatDate(message.date)}</span>
          {message.scriptureRef && <span className="text-cyan-400/70">· {message.scriptureRef}</span>}
        </div>
      </div>
