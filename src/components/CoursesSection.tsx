    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 pt-8 pb-10">
      ...
      <div className="text-center mb-8">
        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-sky-500/10 border border-sky-500/30 mb-4">
          ...
          <span className="text-xs font-bold tracking-[0.2em] text-sky-400 uppercase">Learning Hub</span>
        </div>
...
function CourseCard({ course, onEdit, onDelete, onToggleLesson }: { 
  ...
  return (
    <div className="bg-slate-800/40 border border-slate-700/50 rounded-2xl overflow-hidden hover:border-sky-500/40 transition-all group shadow-xl flex flex-col h-full">
      <div className="aspect-video bg-slate-700 relative overflow-hidden flex-shrink-0">
        {course.thumbnail ? (
          ...
          <div className="w-full h-full flex flex-col items-center justify-center bg-gradient-to-br from-sky-900/40 to-cyan-900/40">
            <Monitor size={48} className="text-sky-500/40 mb-2" />
          </div>
        )}
        <div className="absolute top-3 right-3 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          <button onClick={() => onEdit(course)} className="p-2 rounded-lg bg-black/60 text-white hover:bg-sky-500 transition-colors"><Pencil size={14}/></button>
        ...
        <div className="absolute bottom-3 left-3 flex gap-2">
          ...
          <span className={`px-2 py-0.5 rounded backdrop-blur-md text-white text-[10px] font-bold uppercase tracking-wider ${
            course.status === 'in-progress' ? 'bg-sky-500/80' : 
            course.status === 'completed' ? 'bg-sky-500/80' : 'bg-slate-600/80'
          }`}>
        ...
        <div className="space-y-4 flex-1">
          ...
          <div className="space-y-2">
            <div className="flex justify-between text-[10px] font-bold text-slate-500 uppercase">
              ...
              <span className="text-sky-400">{course.progressPercent}%</span>
            </div>
            <div className="h-1.5 bg-slate-700 rounded-full overflow-hidden">
              <div className="h-full bg-sky-500 transition-all" style={{ width: `${course.progressPercent}%` }} />
            </div>
          ...
          <div className="space-y-2 border-t border-slate-700/50 pt-4">
            ...
            <div className="space-y-1.5 max-h-32 overflow-y-auto no-scrollbar">
              {course.lessons.slice(0, 5).map(lesson => (
                ...
                >
                  {lesson.completed ? <CheckCircle2 size={14} className="text-sky-500" /> : <Circle size={14} className="text-slate-500" />}
              ...
              {total > 5 && (
                <button onClick={() => onEdit(course)} className="w-full text-center text-[10px] font-bold text-sky-400 hover:text-sky-300 py-1 transition-colors">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-4">
            ...
            >
              {thumbnail ? (
                ...
                <div className="flex flex-col items-center gap-2">
                  <Plus className="text-slate-600 group-hover:text-sky-500 transition-colors" />
            ...
            <input 
              ...
              className="w-full px-4 py-2 bg-slate-800 border border-slate-700 rounded-xl text-white text-sm focus:ring-2 focus:ring-sky-500/40" 
            />
            <input 
          ...
          <div className="flex flex-col">
            ...
            <div className="flex gap-2 mb-3">
              ...
              />
              <button onClick={handleAddLesson} className="p-1.5 bg-sky-500 text-white rounded-lg hover:bg-sky-600 transition-colors">
                <Plus size={16}/>
              </button>
            </div>
            ...
            <button 
              ...
              className="w-full py-3 bg-sky-500 text-white font-bold rounded-xl hover:bg-sky-600 transition-all mt-4"
            >
              Save Course
            </button>
          </div>
